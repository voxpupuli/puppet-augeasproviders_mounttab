...
describe provider_class do
  ...
  context 'with empty file' do
    ...
    it 'creates new entry without options' do
      apply!(Puppet::Type.type(:mounttab).new(
               name: '/mnt',
               device: '/dev/myvg/mytest',
               fstype: 'ext4',
    ...
    it 'creates two new entries' do
      ...
      aug_open(target, 'Fstab.lns') do |aug|
        aug.match("./*[vfstype='ext4']").size.should == 2
      end
    end
  end
  ...
  context 'with full file' do
    ...
    it 'updates device without changing dump or pass' do
    ...
    describe 'when updating options' do
      ...
      it 'adds various complex options' do
        apply!(Puppet::Type.type(:mounttab).new(
                 name: '/home',
    ...
    describe 'when updating dump' do
      ...
      context 'when nooptions is supported', if: nooptions_supported? do
        ...
        it 'adds options first, then dump' do
          apply!(Puppet::Type.type(:mounttab).new(
                   name: 'swap',
                   dump: 1,
                   target: target,
    ...
    describe 'when updating pass' do
      ...
      context 'when nooptions is supported', if: nooptions_supported? do
        ...
        it 'adds options and dump first, then pass' do
          apply!(Puppet::Type.type(:mounttab).new(
                   name: 'swap',
                   pass: 1,
                   target: target,
